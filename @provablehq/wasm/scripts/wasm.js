...
function execWasmPack() {
  ...
  if (!target) {
    throw new Error("The build target must be specified (e.g., 'mainnet' or 'testnet')");
  }
  ...
  execSync(`RUSTFLAGS='' wasm-pack build --release --target web --features 'browser,${target}'`, { stdio: 'inherit' });
  ...
  const targetDir = path.resolve(`../pkgs/aleo_wasm_${target}/pkg`);
