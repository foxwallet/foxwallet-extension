...
export const ConfirmMnemonicStep = (props: { onConfirm: () => void, mnemonic: string }) => {
  ...
  // }, [wordList]);
  const [partialWordList, initialOtherWords, placeholderIndexList] = useMemo(() => {
  ...
  const onWordCancel = useCallback((i: number) => {
  ...
  const onWordSelect = useCallback((word: string) => {
    setAnswerWordList((prev) => {
      const newList = [...prev];
  ...
  return <Content>
    <Flex bg={"gray.50"} p={2} borderRadius={"lg"} flexWrap={"wrap"} minH={12} alignItems={"flex-start"} justifyContent={"flex-start"} borderColor={isValid ? "green.300" : "red.300"} borderWidth={"1px"}>
      {
        answerWordList.map((word, index) => {
          const option = placeholderIndexList.includes(index);

          if (word === PLACEHOLDER) {
            return <Box key={index} width={"32%"} mr={(index + 1) % 3 === 0 ? "0" : "2%"} mt={index >=3 ? 2 : 0} />
          }

          return (
            <Box display={"flex"} key={index} px={2} py={2} borderRadius="lg" bg={"gray.100"} justifyContent={"center"} alignItems={"center"}  width={"32%"} mr={(index + 1) % 3 === 0 ? "0" : "2%"} mt={index >=3 ? 2 : 0} onClick={option ? () => onWordCancel(index) : undefined}>
              <Text wordBreak={"break-word"} fontWeight={"bold"} unselectable={"on"} data-child={index}>{word}</Text>
              {
    ...
    <Grid templateColumns="repeat(3, 1fr)" gap={2} alignSelf={"stretch"} p={2} borderRadius={"lg"}>
      {otherWordList.map((otherWord, index) => (
        <GridItem display={"flex"} key={index} px={2} py={2} borderRadius="lg" bg={"orange.100"} justifyContent={"center"} alignItems={"center"} flexWrap={"wrap"} data-child={index} onClick={() => onWordSelect(otherWord)}>
          <Text wordBreak={"break-word"} fontWeight={"bold"} color={"orange.500"} unselectable={"on"} data-child={index}>{otherWord}</Text>
        </GridItem>
      ))}
    </Grid>
    <Flex position={"fixed"} bottom={10} left={4} right={4} >
      <Button flex={1} onClick={onConfirm} isDisabled={!isValid}>{"Confirm"}</Button>
    </Flex>
  </Content>
};
