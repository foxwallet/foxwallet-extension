...
export type CustomGasSettingData = {
...
type CustomGasViewProps = {
  ...
  gasUnit: string;
  feeType?: GasFeeType;
  onConfirm: (data: CustomGasSettingData) => void;
};
...
interface GasSettingStepProps {
  ...
  onConfirm: (data: CustomGasSettingData) => void;
  customGasSettingData?: CustomGasSettingData;
}

export const GasSettingStep = (props: GasSettingStepProps) => {
  const { onConfirm, uniqueId, customGasSettingData = {} } = props;
  const { t } = useTranslation();
  const {
    gasFee,
    isCustom = false,
  ...
  const gradeShow = !!gasGrade && !!gasFee;
  return (
    <Content h={"400px"}>
      <Flex
        direction={"column"}
      ...
      >
        ...
        <CustomGasView
          ...
          isAdvanced={advanceSettings || !gradeShow}
          setAdvanced={() => {
